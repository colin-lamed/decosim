...
import Decosim.Sim.Types (Bar(Bar), Meters(Meters), He, N2, O2)


-- | paO2 below which leads to hypoxia
o2HypoxiaAta :: Tagged O2 Bar
o2HypoxiaAta = tagged $ Bar 0.1

-- | and 1.4 for cold water
o2ToxicityAta :: Tagged O2 Bar
o2ToxicityAta = tagged $ Bar 1.6
...
at :: forall a. Array a -> Int -> a
at array i = unsafePartial $ A.unsafeIndex array i



surfaceN2Pa :: Tagged N2 Bar
surfaceN2Pa = HA.calculatePa (tagged 0.79) zero

surfaceHePa :: Tagged He Bar
surfaceHePa = HA.calculatePa (tagged 0.0) zero
...
percentO2 Air      = tagged $ 0.21
percentO2 NitroxI  = tagged $ 0.32
percentO2 NitroxII = tagged $ 0.36
percentO2 Oxygen   = tagged $ 1.0
percentO2 TrimixI  = tagged $ 0.2
percentO2 TrimixII = tagged $ 0.12

percentN2 :: GasMix -> Tagged N2 Number
percentN2 Air       = tagged $ 0.79
percentN2 NitroxI   = tagged $ 0.68
percentN2 NitroxII  = tagged $ 0.64
percentN2 Oxygen    = tagged $ 0.0
percentN2 TrimixI   = tagged $ 0.45
percentN2 TrimixII  = tagged $ 0.28

percentHe :: GasMix -> Tagged He Number
percentHe g =
  tagged $ 1.0 - (untagged $ percentO2 g) - (untagged $ percentN2 g)
...
showModel model =
    "{ gasMix: " <> show model.gasMix <> ", depth: " <> show model.depth <> ", tissues: {" <> L.intercalate "," (map showRecord model.tissues) <> "} }"
...
calculateDecoStopDepth sad =
    multiplesOf3 * 3
  where
    multiplesOf3' = I.floor (unwrap sad) / 3
    multiplesOf3 = multiplesOf3' + (if (unwrap sad Math.% 3.0) == 0.0 then 0 else 1)
...
decoStops model =
  ...
  where
            ...
            then Right acc
               ...
               in if nextStop.depth >= stop.depth
                   ...
                   then if stop.gasMix == Air
                        then Left ("No stop available! Waiting at " <> showRecord stop <> " will lead to a deeper stop " <> showRecord nextStop)
                             ...
                             in if minOperatingDepth nextGasMix < stop.depth && maxOperatingDepth nextGasMix > stop.depth
                               ...
                               else Left ("No stop available! Waiting at " <> showRecord stop <> " will lead to a deeper stop " <> showRecord nextStop <> " and can't switch to " <> show nextGasMix <> " - outside operating depth")
    ...
    decoStopTime model =
      ...
      in if depth <= zero then zero
         else let model1              = updateModel depth zero model
...
noDecoTimeAtDepth model depth =
  noDecoTime (updateModel depth zero model)
...
min' :: forall a. Ord a => Array a -> a
min' a = foldl min (unsafePartial head a) a

max' :: forall a. Ord a => Array a -> a
