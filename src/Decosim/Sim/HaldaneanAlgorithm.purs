...
import Decosim.Sim.Types(Bar(Bar), Meters(Meters), He, N2, extract)
...
barToDepth bar =
  Meters $ 10.0 * (unwrap $ bar - surfaceAta)

depthToBar :: Meters -> Bar
depthToBar depth =
  Bar $ (unwrap depth / 10.0 + unwrap surfaceAta)

toTissueConf :: forall t. Tagged t Bar -> Minutes -> TissueConf t
--toTissueConf :: _
toTissueConf surfacePa halflife =
    { halflife : tagged halflife
    , a        : tagged a
    , b        : tagged b
    , mo       : tagged mo
    , k        : tagged k
  ...
  where
    ...
    mo  = over Bar (\pa -> pa / b + a) (untagged surfacePa)
    k   = -Math.log(0.5) / tht
...
calculatePa :: forall t. Tagged t Number -> Meters -> Tagged t Bar
calculatePa partialPercentage depth =
  tagged $ over Bar (\d -> untagged partialPercentage * d) (depthToBar depth - pw)


-- | The reverse of calculatePa
calculateDepthForGivenPa :: forall t. Tagged t Number -> Tagged t Bar -> Meters
calculateDepthForGivenPa partialPercentage pa =
  barToDepth $ Bar (extract pa / untagged partialPercentage) + pw
...
calculatePt :: forall t. Tagged t Bar -> Tagged t Bar -> Minutes -> TissueConf t -> Tagged t Bar
calculatePt previousPt' pa' timeStep' tissueConf =
    tagged $ Bar $ previousPt + (pa - previousPt) * (1.0 - Math.exp(-k * timeStep))
  where
    previousPt = extract previousPt'
    pa         = extract pa'
    timeStep   = unwrap timeStep'
    k          = untagged tissueConf.k

-- | This is the schreiner equation.
-- | We get the 'Instantaneous Equation' calculatePt, when `r=0`
calculatePt2 :: forall t. Tagged t Bar -> Tagged t Bar -> Number -> Tagged t Number -> Minutes -> TissueConf t -> Tagged t Bar
calculatePt2 initPt' initPa' descentRate partialPercentage timeStep' tissueConf =
    tagged $ Bar $ initPa + r * (1.0 - 1.0 / k) - (initPa - initPt - r / k) * Math.exp(- k * timeStep)
  where
    r        = descentRate * untagged partialPercentage
    initPa   = extract initPa'
    initPt   = extract initPt'
    timeStep = unwrap timeStep'
    k        = untagged tissueConf.k
...
calculateTimeToSaturation :: forall t. Tagged t Minutes -> Tagged t Bar -> Tagged t Bar -> Tagged t Bar -> Maybe Minutes
calculateTimeToSaturation tissueHalfLife mo pt pa =
  if    pt /= pa   -- else at equilibrium
     ...
     && timeToSaturation >= zero -- i.e. we're already saturated
    then Just timeToSaturation
    else
      Nothing
  where
    aux = extract (mo - pa) / extract (pt - pa)
    timeToSaturation = Minutes $ Math.log(aux) * extract tissueHalfLife / Math.log(0.5)
...
decoStopTime stopDepth n2Per hePer n2Conf heConf n2Pt hePt =
    foldl max zero $ A.catMaybes [map toNextMin n2DecoTime, map toNextMin heDecoTime]
  where
    n2PaDecoDepth = calculatePa n2Per stopDepth
    hePaDecoDepth = calculatePa hePer stopDepth
    nextStop      = max zero (stopDepth - stopDelta)
    toleratedN2Pa = tagged $ untagged n2Conf.mo * depthToBar nextStop
    toleratedHePa = tagged $ untagged heConf.mo * depthToBar nextStop
...
calculateSafeAscentDepth :: forall t. Tagged t Bar -> TissueConf t -> Meters
calculateSafeAscentDepth pt' tissueConf =
    barToDepth $ Bar (pt / mo)
  where
    pt = extract pt'
    mo = extract tissueConf.mo
...
ascentCeiling' n2Pt' hePt' n2Conf heConf =
  ...
  where
    n2Pt = extract n2Pt'
    hePt = extract hePt'
    n2A = untagged n2Conf.a
    n2B = untagged n2Conf.b
    heA = untagged heConf.a
    heB = untagged heConf.b
